...
async function InitialRegionsSection() {
  ...
  try {
    const data = await fetchAirQualityBySido(defaultSido);
    const items = data?.response?.body?.items ?? [];
    return (
      <ClientRegions
        initialSido={defaultSido}
        initialSidoItems={items}
      />
    );
  } catch (e) {
    console.error('서버에서 지역별 데이터 가져오기 실패, 클라이언트 위임:', e);
  ...
  return (
    <MainLayout>
      <div className="container mx-auto p-6 space-y-6">
        ...
        <p className="text-muted-foreground">전국 → 시도 → 시군구 단계로 선택하고, 리스트 또는 지도 방식으로 대기질 정보를 확인하세요.</p>

        <DashboardCard title="지역 선택 및 현황" fullWidth>
